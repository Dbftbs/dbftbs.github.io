...
namespace :theme do
  ...
  #        The theme must be installed into your blog framework.
  #
  # Examples
  #
  ...
  task :install do
    ...
    packaged_theme_path = blog::Path.build(:theme_packages, :node => name)
    ...
    packaged_theme_files.each do |filename|
      file_install_path = File.join(blog::Path.base, filename)
  ...
  # The theme must be structured using valid blog API.
  ...
  task :package do
    name = ENV["name"].to_s.downcase
    theme_path = blog::Path.build(:themes, :node => name)
    asset_path = blog::Path.build(:theme_assets, :node => name)
    ...
    packaged_theme_path = blog::Path.build(:themes, :root => blog::Path.build(:theme_packages, :node => name))
    mkdir_p packaged_theme_path
    cp_r theme_path, packaged_theme_path
    
    ## Mirror theme's asset directory
    packaged_theme_assets_path = blog::Path.build(:theme_assets, :root => blog::Path.build(:theme_packages, :node => name))
    ...
    open(blog::Path.build(:theme_packages, :node => "#{name}/packager.yml"), "w") do |page|
      page.puts packager.to_yaml
    end
    
    puts "=> '#{name}' theme is packaged and available at: #{blog::Path.build(:theme_packages, :node => name)}"
  end
...
def theme_from_git_url(url)
  tmp_path = blog::Path.build(:theme_packages, :node => "_tmp")
  abort("rake aborted: system call to git clone failed") if !system("git clone #{url} #{tmp_path}")
  manifest = verify_manifest(tmp_path)
  new_path = blog::Path.build(:theme_packages, :node => manifest["name"])
